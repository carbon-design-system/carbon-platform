...
module.exports = {
  A: {
    'Action labels': {
      Add: {
        desc:
          'Takes an existing object and uses it in a new context (for example,' +
          ' adds an item to the cart, adds a user to a group, or adds a document to a folder).',
        subtext:
          'Where appropriate, combine add with the object (for example, Add user or Add role).' +
          ' Compare [Create](#create), [Insert](#insert), [New](#new), and [Upload](#upload).' +
          ' See also the [Add pattern](/patterns/common-actions#add).'
      },
      Apply: {
        desc: 'Saves changes without closing the dialog. These properties often affect subsequent system behavior.',
        subtext: 'Use instead of Save changes. Compare [Save](#save) and [Save as](#save-as).'
      },
      Approve: {
  ...
  B: {
    'Action labels': {
      Back: {
        ...
        subtext: 'Use instead of Previous. Compare [Next](#next) and [Finish](#finish).'
      },
      Browse: {
        desc:
          'Assists the user in selecting a file (for example, on a button or link next to an entry field).' +
          ' Typically opens a secondary window where the user can locate and select the desired directory and file.',
        subtext: null
      }
    }
  },
  C: {
    'Action labels': {
      Cancel: {
        ...
        subtext:
          'Warn the user of any possible negative consequences of stopping an action from progressing,' +
          ' such as data corruption. Compare [Reset](#reset). See also the [Cancel pattern](/patterns/common-actions#cancel).'
      },
      Clear: {
        desc:
          'This action clears all the fields or selections. Also deletes the contents of a document, such as a log.' +
          ' Typically the default selection or value is re-established for controls that always have a selection or value, ' +
          'such as [radio buttons](/components/radio-button/usage).',
        subtext:
          'Where appropriate, combine clear with the object (for example, Clear fields or Clear all). ' +
          'Compare [Delete](#delete) and [Remove](#remove). See also the [Clear pattern](/patterns/common-actions#clear).'
      },
      Close: {
        desc: 'Closes the current page or window (for example, closing a secondary window containing online help).',
        subtext:
          'Do not use Close alongside OK or Cancel actions. Compare [Cancel](#cancel) and [Done](#done). ' +
          'See also the [Close pattern](/patterns/common-actions#close).'
      },
      Copy: {
        desc: 'Creates new instances of the selected objects in a specific destination.',
        subtext:
          'Combine Copy with the object being copied (for example, Copy folder) or the destination' +
          ' (for example, Copy to clipboard) if there are multiple possibilities. Compare [New](#new). ' +
          'See also the [Copy pattern](/patterns/common-actions#copy).'
      },
      Create: {
        ...
        subtext:
          ...
          'use new to initiate the action and create to apply the user-supplied details or settings to the new object.' +
          ' Compare [Add](#add), [Copy](#copy), [Insert](#insert), and [New](#new).'
      },
      Customize: {
  ...
  D: {
    'Action labels': {
      Delete: {
        desc:
          'Destroys an existing object so that it no longer exists (for example, deletes a file from a directory or' +
          ' deletes a value from a table cell).',
        subtext:
          'Where appropriate, combine Delete with the object (for example, Delete column or Delete row).' +
          ' Compare [Clear](#clear) and [Remove](#remove). See also the [Delete pattern](/patterns/common-actions#delete)'
      },
      Docs: {
        desc: 'Opens a separate window containing the landing page for the product documentation.',
        subtext:
          'Use as link text only for the specific link that points to the product documentation from the console menu bar.' +
          ' Compare [Learn more](#learn-more).'
      },
      Done: {
        desc:
          'Indicates that the user has finished working in an environment (for example, editing templates) and wants' +
          ' to return to where he or she came from.',
      ...
      Drop: {
        ...
        subtext: 'In other scenarios, use [Clear](#clear), [Delete](#delete), or [Remove](#remove).'
      }
    }
  },
  E: {
    'Action labels': {
      ...
      Export: {
        desc:
          'Saves data in a different format external to the system. Typically opens a secondary window for ' +
          'the user to specify the file type and destination (for example, storing table data as a set of comma-separated values).',
  ...
  F: {
    'Action labels': {
      ...
      Find: {
        desc:
          'Moves the cursor to the next element matching the specified criteria (for example,' +
          ' view the next occurrence of a specific word within an email message).',
  ...
  L: {
    'Action labels': {
      ...
      'Learn more': {
        ...
        subtext:
          "If space permits, combine Learn more with meaningful text that describes the content you're pointing to." +
          ' For example, if your user needs some best practices to manage apps in multiple regions, you could use Learn more about regions.'
      },
      'Log in': {
        desc:
          'Enters a site or application. This choice typically opens a form for entry of credentials.' +
          ' Also used on the submission button after users enter their credentials.',
        subtext:
          'Use instead of Sign in. This is to make it visually distinct from Sign up. ' +
          'These options are often side by side and the different words allow for quick recognition. ' +
          'Compare [Log out](#log-out). See also the [Login pattern](/patterns/login-pattern).'
      },
      'Log out': {
  ...
  N: {
    'Action labels': {
      New: {
        desc:
          'Starts the creation of a new object.' +
          ' New either creates the object immediately or opens a dialog or set of fields where the user can enter properties.',
        subtext:
          'Combine new with the object to create (for example, New user or New column). Compare [Add](#add), ' +
          '[Copy](#copy), [Create](#create), [Insert](#insert), and [Save as](#save-as).'
      },
      Next: {
  ...
  O: {
    'Action labels': {
      OK: {
        ...
        subtext:
          'Best practice is to use a label corresponding to the specific action (for example, [Save](#save) or [Close](#close)' +
          ' or [Delete](#delete)). Use OK only when such a label is not available. Write as shown: two letters, both uppercase.'
      }
    }
  },
  P: {
    'Action labels': {
      ...
      Preview: {
        desc:
          'Shows how an object or content will appear with formatting applied before the content is published or distributed.' +
          ' Alternatively, provides an incomplete display of an existing object without leaving the current context.',
        subtext: null
      },
      Print: {
  ...
  R: {
    'Action labels': {
      ...
      Remove: {
        desc:
          'Removes an object from the current context but the object is not destroyed as a result of the action (for example,' +
        ...
        subtext:
          ...
          'Compare [Clear](#clear) and [Delete](#delete). See also the [Remove pattern](/patterns/common-actions#remove).'
      },
      Reply: {
      ...
      Reset: {
        desc:
          'Reverts values back to their last saved state. The last saved state includes the values stored the last time the user clicked Apply.' +
          ' Does not close the dialog or window.',
        subtext:
          'Compare [Cancel](#cancel), [Restore](#restore), [Restore defaults](#restore-defaults), and [Undo](#undo). ' +
          'See also the [Reset pattern](/patterns/common-actions#reset).'
      },
      Restore: {
  ...
  S: {
    'Action labels': {
      ...
      'Save as': {
        desc:
          'Creates a new object based on the state of the object currently being viewed.' +
          ' The user names the new object and typically identifies its location.',
        subtext: null
      },
      Search: {
        desc:
          'Returns all objects (for example, files, names, or documents) within a defined set' +
          ' (for example, in a folder, directory, database, or the internet) that match some specified criteria.',
      ...
      Sort: {
        ...
        subtext:
          "Likely used only as a tooltip on an icon button. Can be used without 'ascending' or 'descending' " +
          'only if the order can be provided to a screen reader in the code for accessibility.'
      },
      Start: {
      ...
      'Submit an idea': {
        ...
        subtext: 'Use only as link text on the Support widget from the console menu bar.'
      }
    }
  },
  T: {
  ...
  U: {
    'Action labels': {
      Undo: {
        desc:
          'Reverts to the state before the most recent changes made by the user. Repeated use successively reverts' +
          ' to prior states in reverse chronological order. Applies to changes in data and not to changes made to the view.',
        subtext:
