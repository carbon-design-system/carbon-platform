...
Additionally, design kits defined __inside library objects__ can be specified as a schema reference. See [schema references](#schema-references).
...
For certain resource keys, platform allows the object to be defined as a reference to another existing resource instead of writing the actual value:
```yml path="Example carbon.yml"
designKits:
  sample-design-kit:
...
An absolute url that points to a resource contained in a yml file , e.g.: https://unpkg.com/@carbon-platform/resources/carbon.yml#/designKits/carbon-g10-sketch
...
A relative path to a resourced contained in a yml file within the current repo, e.g.: './design-kits.yml#/designKits/sample-design-kit'
...
Reference to a resource contained in the same file where the referencing property is being defined, e.g.: '#/designKits/sample-design-kit'
...
Note that it must be explicitly identified in the schema that a certain object or property accepts references for this to work.
